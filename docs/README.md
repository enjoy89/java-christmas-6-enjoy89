...
- [ ]  프로그램 시작 문구를 출력한다.
- [ ]  방문 날짜에 받는 이벤트 혜택 미리 보기 문구를 출력한다.
...
- [ ]  객체는 객체스럽게 사용한다.
    - [ ]  객체에서 데이터를 꺼내지(get) 말고 메시지를 던져 일하도록 구조를 짠다.
- [ ]  필드의 수를 줄이기 위해 노력한다.
- [ ]  성공하는 케이스 뿐만 아니라 예외에 대한 케이스로 테스트한다.
